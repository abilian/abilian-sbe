...
def object_viewers(obj):
    if current_user.has_role('manager'):
        if obj:
            ...
            community_members_id = {user.id:user for user in g.community.members if user.id != obj.creator.id}
            viewers = []
            for view in tracked_views:
                if view.user_id in set(community_members_id):
                    viewers.append({'user':community_members_id[view.user_id],'viewed_at':view.viewed_at})
            return viewers

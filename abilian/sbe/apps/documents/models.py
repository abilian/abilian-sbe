...
from sqlalchemy.types import UnicodeText, Text, Integer
...
class PathAndSecurityIndexable(object):
  ...
  def _indexable_roles_and_users(self):
    ...
    if self.parent:
      ...
      iter_from_root.next()
    allowed = set(o[0] for o in security.get_role_assignements(iter_from_root.next()))
...
class Folder(CmisObject, PathAndSecurityIndexable):
  ...
  def filtered_children(self):
    return security.filter_with_permission(g.user, "read", self.children,
                                           inherit=True)

  @property
  def filtered_subfolders(self):
    return security.filter_with_permission(g.user, "read", self.subfolders,
                                           inherit=True)
...
class BaseContent(CmisObject):
  ...
  content_blob = relationship(Blob, cascade='all, delete',
  ...
  content_length = Column(
    Integer, default=0, nullable=False, server_default=sa.text('0'),
  ...
  content_type = Column(
    Text, default="application/octet-stream",
    info=dict(searchable=True,
...
class Document(BaseContent, PathAndSecurityIndexable):
  ...
  def has_preview(self, size=None, index=0):
    ...
    return converter.has_image(self.content_digest, self.content_type, index,
                               size)
  ...
  preview_blob = relationship(Blob, cascade='all, delete',
